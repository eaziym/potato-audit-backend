...
def fetch_process_reports():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Reports fetched and processed successfully."}
        )
...
def handle_fetch_announcements():
    ...
    target_dir = os.path.join(
        downloadDir, f"{company.replace(' ', '_').upper()}_FY{year}"
    )
...
def fetch_todays_announcements():
    ...
    )  # Save the file in the provided download directory
    ...
    return jsonify(
        {"status": "success", "message": f"Announcements saved to {filename}"}
    )
...
def process_directory():
    ...
    invoice_files = [
        f for f in os.listdir(directory_path) if f.lower().endswith((".pdf", ".png"))
    ]

    if not invoice_files:
        return (
            jsonify({"error": "No invoice files found in the specified directory."}),
            404,
        )
    ...
    processed_path, blobs, mergedTexts = process_invoice_image(
        sample_invoice_path, PROCESSED_FOLDER
    )
    ...
    return jsonify(
        {
            "status": "success",
            "message": "Sample invoice processed. Ready for template creation.",
            "filename": filename,
            "blobs": blobs,
...
def upload_docs():
    ...
    try:
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Great! File processed successfully and saved to {excel_path}",
            }
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def process_selected_pairs():
    ...
    # Track progress
    invoice_files = [
        f for f in os.listdir(directoryPath) if f.lower().endswith((".pdf", ".png"))
    ]
    ...
    for index, invoice_file in enumerate(invoice_files, start=1):
        ...
        processed_path, invoice_blobs, invoice_merged_texts = process_invoice_image(
            invoice_path, PROCESSED_FOLDER
        )
        ...
        for label_text, (
            ...
            spatial_relationships.items()
        ):  # Assuming spatial_relationships is available
            # Find label box in the current invoice using label_text
            print("Current label: " + label_text)
            label_box, found_label_text = find_coordinates_by_text(
                label_text, invoice_merged_texts, dx, dy, labelVariations, threshold=90
            )
            ...
            closest_box = find_closest_box(
                dx, dy, estimated_value_coords, invoice_merged_texts, omit_box=label_box
            )
            if closest_box:
                ...
                estimated_value_coords = apply_spatial_relationship(label_box, dx, dy)
                closest_box = find_closest_box(
                    dx, dy, estimated_value_coords, invoice_merged_texts
                )
                if closest_box:
                    value_text = closest_box["text"]
                    if dx == 0 and dy == 0:
        ...
        for col, (key, value) in enumerate(info.items(), start=1):
            if key == "File URL":
                ...
                ws[f"{get_column_letter(col)}{row}"] = (
                    f'=HYPERLINK("{value}", "Click Here to Open File")'
                )
            else:
                if value:
    ...
    output_filename = os.path.join(
        directoryPath, "processed_invoices.xlsx"
    )  # Save in the user's directory
...
def get_processed_info():
    directory_path = request.args.get(
        "directory"
    )  # Assume directory path is provided as a query parameter
...
def get_staff():
    ...
    try:
        ...
        staff_data = [
            {"name": row["Staff's Name"], "level": row["Level"]}
            for index, row in df.dropna().iterrows()
        ]  # Drop any rows where data may be missing
...
def get_clients():
    ...
    try:
        df = pd.read_excel(
            "client_names.xlsx", sheet_name="Sheet1", usecols="B", skiprows=0, nrows=620
        )

        client_names = (
            df["Client's Name"].dropna().tolist()
        )  # Convert the data to a list, removing any NaN values
...
level_to_rate = {"AA": 180, "EA": 220, "SA1": 250, "SA2": 300, "AM": 360}
...
def create_booking():
    ...
    process_booking(
        booking_data, rates
    )  # Call the function to process and update Excel
    return jsonify(
        {"status": "success", "message": "Booking processed and saved to Excel."}
    )
